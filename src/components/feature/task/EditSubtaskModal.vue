...
const { editTaskModal, editTask } = storeToRefs(tasksStore);

const { addTagInput, addTag, formData, reset, tagInput, availableTags, formDataToSave, setData } =
  useTaskForm();

watchEffect(() => {
  if (editTask.value && editTaskModal.value) {
    setData(editTask.value);
  }
  if (editTaskModal.value === false) {
    reset();
  }
});
</script>
      ...
      <AppModalHeader>
        ...
        <form>
          ...
          <div class="flex gap-4 justify-end mt-4">
            <AppButton variant="primary" @click="updateTask(formDataToSave)">Save</AppButton>
